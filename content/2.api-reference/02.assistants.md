---
title: Ассистенты (Assistants)
description: Настраивайте собственных ассистентов на базе выбранных моделей и сервисов
---
...
::field-group
  :::field{name="HTTP"}
  GET `/assistants/{id}`:br Возвращает данные конкретного ассистента по его ID
  :::
...
::field-group
  :::field{name="HTTP"}
  PUT `/assistants/{id}`:br Обновляет данные существующего ассистента
  :::
...
::field-group
  :::field{name="HTTP"}
  DELETE `/assistants/{id}`:br Удаляет ассистента по его ID
  :::
