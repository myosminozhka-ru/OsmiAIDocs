...
::field-group
  :::field{name="HTTP"}
  GET `/chatmessage/{id}`
  Возвращает все сообщения для конкретного агента.
  :::
  ...
  Возможные значения: `INTERNAL`, `EXTERNAL`:br`order` (enum) — порядок сортировки
  Возможные значения: `ASC`, `DESC`:br`chatId` (string) — фильтр по ID чата
...
::field-group
  :::field{name="HTTP"}
  DELETE `/chatmessage/{id}`:br Удаляет все сообщения для конкретного агента.
  :::
  ...
  :::field{name="Query-параметры"}
  `chatId` (string) — фильтр по ID чат :br `memoryType` (string) — фильтр по типу памяти (например, `Buffer Memory`) :br `sessionId` (string) — фильтр по ID сессии :br `chatType` (enum) — фильтр по типу чата (`INTERNAL`, `EXTERNAL`) :br `startDate` (date-time) — фильтр по дате начала :br `endDate` (date-time) — фильтр по дате окончания :br `feedbackType` (enum) — фильтр по типу фидбэка (`THUMBS_UP`, `THUMBS_DOWN`) :br `hardDelete` (boolean) — если `true`, то сообщения удаляются также у сторонних сервисов
  :::
::
