...
::field-group
  ...
  `replaceExisting` (boolean | nullable) — заменить существующий загрузчик новыми чанками (не удаляет эмбеддинги из БД векторов)
...
::field-group
  ...
  :::field{name="Тело (application/json):"}
  `id` (string · uuid) — уникальный ID базы документов
  ...
  `vectorStoreConfig` (string) — конфиг БД вектора (JSON-строка)
  
  `embeddingConfig` (string) — конфиг эмбеддинга (JSON-строка)
  
  `recordManagerConfig` (string) — конфиг БД записей (JSON-строка)
...
::field-group
  ...
  Обновляет сведения о базе документов по его ID.
  :::
...
::field-group
  ...
  Удаляться будут только данные, которые были добавлены или обновлены с помощью БД записей, из БД векторов.
  :::
