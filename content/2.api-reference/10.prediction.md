...
::field-group
  ...
  - `question` (string) — Вопрос/сообщение для отправки в поток :br Пример: `"What is artificial intelligence?"`
  - `form` (object) — объект формы, альтернативный question для Agentflow V2 :br Пример: `{"title":"Example","count":1}`
  - `streaming` (boolean) — Включить потоковые ответы для вывода в реальном времени :br По умолчанию: `false`
  - `overrideConfig` (object) — Переопределить конфигурацию потока и передать переменные при выполнении :br Пример: `"sessionId":"user-session-123","temperature":0.7,"maxTokens":500,"vars":{"user_name":"Alice"}`
  - `history` (object [] ) — Предыдущие сообщения разговора для контекста :br Пример: `[{"role":"apiMessage","content":"Hello!..."} , {"role":"userMessage","content":"Hi, my name is Sarah..."}]`
  - `uploads` (object [] ) — файлы для загрузки (изображения, аудио, документы и т. п.) в data URL / base64 :br Пример: `[{"type":"file","name":"example.png","data":"data:image/png;base64,...","mime":"image/png"}]`
  - `humanInput` (object) — возврат на «чекпоинт», ожидавший человеческий ввод, и продолжение исполнения :br Пример: `{"type":"reject","feedback":"Include more emoji"}`
  :::
::
