...
::field-group
  :::field{name="HTTP"}
  POST `/vector/upsert/{id}`:br Выполняет upsert (вставку/обновление) векторных эмбеддингов документов в рамках указанного чат-флоу.
  :::
  ...
  - `stopNodeId` (string) — если используется несколько узлов векторного хранилища, укажите ID узла, в который сохранять векторы :br Пример: `"node_1"`
