...
**Authentication**: Refer [Authentication for Flows](https://docs.flowiseai.com/configuration/authorization/chatflow-level)
...
{% hint style="warning" %}
Variables must be created first before you can override it. Refer to [Variables](https://docs.flowiseai.com/using-flowise/variables)
{% endhint %}
...
Upload images for visual analysis when your flow supports image processing. Refer to [Image](https://docs.flowiseai.com/uploads#image) for more reference.
...
Upload audio files for speech-to-text processing. Refer to [Audio](https://docs.flowiseai.com/uploads#audio) for more reference.
...
Upload files to have LLM process the files and answer query related to the files. Refer to [Files](https://docs.flowiseai.com/uploads#files) for more reference.

### Human Input

To resume the execution from a previously stopped checkpoint, `humanInput` needs to be provided. Refer [Human In The Loop](https://docs.flowiseai.com/tutorials/human-in-the-loop) for details.
