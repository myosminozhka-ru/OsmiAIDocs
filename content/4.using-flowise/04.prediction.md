...
**Base URL:** <http://localhost:3000> (или URL вашей инстанции OSMI AI) :br **Endpoint:** POST /api/v1/prediction/\:id :br **Аутентификация:** см. раздел [Authentication for OSMI AI](/configuration/auth/chatflow-level)
...
OSMI предоставляет официальные SDK для Python и TypeScript/JavaScript: :br Установка Python
...
Подробности смотрите в разделе API справочник по [Prediction Endpoint](https://docs.myosmi-ai.fvds.ru/api-reference/prediction).

## Базовый URL и аутентификация

**Base URL:** <http://localhost:3000> (или URL вашей инстанции OSMI AI) :br **Endpoint:** POST /api/v1/prediction/\:id :br **Аутентификация:** см. раздел “Authentication for OSMI AI”
...
| Параметр | Тип     | Обязательно                  | Описание                                                   |
| -------- | ------- | ---------------------------- | ---------------------------------------------------------- |
|          | string  | Да                           | Сообщение или вопрос, который отправляется в поток         |
|          | object  | Либо `question`, либо `form` | Объект формы, который отправляется в поток                 |
|          | boolean | Нет                          | Включить потоковую обработку ответов (по умолчанию: false) |
|          | object  | Нет                          | Переопределить настройки потока                            |
|          | array   | Нет                          | Предыдущие сообщения беседы                                |
|          | array   | Нет                          | Файлы для загрузки (изображения, аудио и т. д.)            |
|          | object  | Нет                          | Вернуть обратную связь от человека и продолжить выполнение |
