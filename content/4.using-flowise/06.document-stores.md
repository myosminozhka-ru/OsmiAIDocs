---
title: База документов  (Document Stores)
...
## 1. Добавьте базу документов (Document Store)

Начните с добавления базы документов Document Store и задайте ему имя. В нашем случае — «LibertyGuard Deluxe Homeowners Policy».
...
2. **БД векторы (Vector Store):** затем определяем, в каком БД векторе \*\*\*\*будет храниться наш набор данных.
3. **Менеджер записей (Record Manager) (опционально):** наконец, при необходимости можно внедрить менеджер записей — компонент, обеспечивающий функции управления набором данных внутри БД вектора.
...
**Понимание размерности эмбеддингов и БД вектора (Understanding Embeddings/Vector Store dimensions)**

Количество размерностей в индексе \*\*\*\*БД вектора определяется используемой моделью эмбеддинга при обновлении данных, и наоборот. Каждая размерность отражает определённую особенность или концепцию в данных — например, тему, эмоциональную окраску или другой аспект текста.
...
Для корректной работы модели эмбеддинга и индекса БД векторов необходимо согласование размерности. Количество размерностей эмбеддингов и вектора в базе документов должно совпадать. Несовпадение приводят к ошибкам при обновлении данных (upsert), так как БД вектор предназначен для обработки векторов фиксированного размера, определённого выбранной моделью эмбеддинга.
...
БД вектор — это специальный тип базы данных, предназначенный для хранения векторных эмбеддингов. Мы можем настроить параметры, например «top K», который определяет максимальное количество наиболее похожих фрагментов, извлекаемых из БД вектора в ответ на запрос.
...
Менеджер записей (Record Manager) — это опциональный, но очень полезный элемент процесса обновления (upsert). Он позволяет вести учёт всех фрагментов, добавленных в БД векторов, что даёт возможность эффективно добавлять или удалять фрагменты по мере необходимости.

Другими словами, любые изменения в ваших документах при новом обновлении не приведут к дублированию векторных эмбеддингов в базу документов.
...
## 7. **Загрузка данных в БД векторов (Upsert Your Data to a Vector Store)**

Для начала процесса обновления и передачи данных в БД векторов нажмите кнопку «Upsert».
...
С помощью узла Agent вы можете добавить базу документов:
...
Для деталей смотрите ссылку на API справочник по [базе документов](/api-reference/document-store) (Document Store API Reference).
