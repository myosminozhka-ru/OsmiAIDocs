---
title: Upsertion
description: Upsert — это процесс загрузки и обработки документов во векторное
  хранилище (Vector Store), который служит основой для систем RAG (Retrieval
  Augmented Generation)
---

Существует два основных способа выполнения upsert в векторное хранилище:

- [Хранилища документов](/using-flowise/document-stores) (рекомендуется)
- Upsert через Chatflow

Мы настоятельно рекомендуем использовать хранилища документов, так как они предоставляют единый интерфейс для работы с RAG-конвейерами — получение данных из разных источников, стратегия разбиения на части (chunking), загрузка в векторную базу данных, синхронизация с обновлёнными данными. В этом руководстве мы рассмотрим другой метод — Upsert через Chatflow. Это более старая методика, применявшаяся до появления хранилищ документов.

Подробности см. в документации к [Vector Upsert Endpoint API.](/api-reference/vector-upsert)

### Понимание процесса обновления или вставки данных (Understanding the upserting process)

Chatflow позволяет создавать поток, который может выполнять как обновление/вставку данных (upserting), так и процесс запросов с использованием RAG (Retrieval-Augmented Generation). Оба эти процесса могут работать независимо друг от друга.
...
\- Менеджер записей (Record Manager) — опционально

Все эти элементы подробнее описаны в разделе «[Хранилища документов](/using-flowise/document-stores)» — обращайтесь туда за дополнительной информацией.
...
![](/assets/Picture1.png)![](/assets/image%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\).png){width="563"}

Также процесс upsert можно выполнить через API:

![](/assets/image%20\(2\)%20\(1\)%20\(1\)%20\(1\)%20\(1\).png){width="563"}

## **Базовый URL и аутентификация**

- Базовый URL: http\://localhost:3000 (или URL вашего экземпляра OSMI)
- Точка входа: POST /api/v1/vector/upsert/\:id
- Аутентификация: см. раздел «[Аутентификация для потоков](/configuration/Auth/chatflow-level)»

## **Методы запроса (Request Methods)**

API поддерживает два типа методов в зависимости от конфигурации вашего chatflow:

1. **Form Data (загрузка файлов)** Используется, если ваш chatflow содержит загрузчики документов с возможностью загрузки файлов.
...
![](/assets/image%20\(3\)%20\(1\)%20\(1\)%20\(1\).png)

### Загрузчики документов с возможностью загрузки файлов (Document Loaders with File Upload)
...
### **Загрузчики документов без загрузки файлов (Document Loaders without File Upload)**
