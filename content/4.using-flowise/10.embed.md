---
title: Настройка и встраивание чат-виджета (Embed)
...
![](/assets/image%20\(8\)%20\(2\)%20\(1\)%20\(1\).png)

## **Настройка виджета (Widget Setup)**

В видео показано, как внедрить скрипт виджета на любую веб-страницу.

## **Использование конкретной версии**

Вы можете указать, какую версию файла web.js из пакета OSMI-embed использовать. Полный список версий доступен по ссылке: нужна ссылка
...
Начиная с версии OSMI v2.1.0 изменён механизм работы потоковой передачи (streaming). Если ваша версия OSMI ниже этой, встроенный чат-бот может не получать сообщения.

Вы можете либо обновить OSMI до версии v2.1.0 и выше, либо, если по каким-то причинам обновлять OSMI не хотите, указать последнюю версию OSMI-embed из ветки v1.x.x. Последняя поддерживаемая версия файла web.js — v1.3.14.

Пример использования: <https://cdn.jsdelivr.net/npm/OSMI-embed@1.3.14/dist/web.js>

## Chatflow Config

Можно передать объект `chatflowConfig`, чтобы переопределить существующую конфигурацию (аналогично API).

Вы можете передать объект chatflowConfig в формате JSON, чтобы переопределить существующие настройки. Это то же самое, что и в API, описанном в этом руководстве.
...
## **Конфигурация наблюдателя (Observer Config)**
...
1\. Форкните репозиторий osmi Chat Embed.
