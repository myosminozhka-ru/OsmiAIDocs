---
title: Загрузка файлов (Uploads)
description: Загрузка изображений, аудио и других файлов
---

OSMI позволяет загружать изображения, аудио и другие файлы прямо из чата. В этом разделе вы узнаете, как включить и использовать эти функции.
...
⚠️ Обработка изображений работает только с определенными цепочками/агентами в Chatflow.
...
::static-image{image="/assets/Screenshot 2024-02-29 011714.png"}
::
...
## **Преобразования речи в текст (Audio)**

В настройках Chatflow вы можете выбрать модуль преобразования речи в текст. Поддерживаются следующие интеграции:
...
::static-image{image="/assets/Screenshot 2024-02-29 012538.png"}
::
...
### **Загрузки файлов RAG (RAG File Uploads)**

Вы можете быстро обновлять или добавлять файлы в векторное хранилище по мере необходимости. Чтобы включить загрузку файлов, убедитесь, что выполнены следующие условия:

В чатфлоу должен быть добавлен векторный хранилище, поддерживающее загрузку файлов.
...
Если в чатфлоу используется несколько векторных хранилищ, загрузка файла возможна только для одного из них одновременно.Необходимо подключить хотя бы один узел загрузчика документов к входу документов этого векторного хранилища.
...
![](/assets/image%20\(2\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\).png)

Вы можете загрузить один или несколько файлов в чат:

::static-image{image="/assets/Screenshot 2024-08-26 170456.png"}
::
...
- Метаданные содержат `osmi_chatId`, и значение равно текущему идентификатору сессии чата.
- Метаданные не содержат `osmi_chatId`.
...
### **Полная загрузка файлов (Full File Uploads)**
...
Чтобы включить полную загрузку файлов, перейдите в настройки Chatflow, откройте вкладку «File Upload» и активируйте переключатель.
...
![](/assets/image%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(2\).png)![](/assets/image%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(2\)%20\(1\).png)

Обратите внимание: если в вашем чатфлоу используется узел шаблона запроса (Chat Prompt Template), необходимо создать входные данные из «Format Prompt Values», чтобы передать данные файла. Указанное имя входа (например, {file}) должно быть включено в поле «Human Message».

![](/assets/chat-prompt-template-file-attachment.jpg)
...
Как видно из примеров, загрузка требует строки в формате base64. Чтобы получить строку base64 для файла, используйте API «[Create Attachments](/api-reference/attachments)».

### **Разница между полной загрузкой и загрузкой RAG (Difference between Full & RAG Uploads)**
