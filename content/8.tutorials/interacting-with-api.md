...
Этот урок демонстрирует, как LLM можно использовать для выполнения вызовов API через вызов инструментов.
...
## Инструменты для запросов (Request Tools)

Доступно 4 инструмента запросов, которые можно использовать. Это позволяет LLM вызывать инструменты GET, POST, PUT и DELETE по мере необходимости.
...
Затем добавьте узел Agent. В этой настройке агент настроен с четырьмя основными инструментами: GET, POST, PUT и DELETE. Каждый инструмент предназначен для выполнения конкретного типа API-запроса.

#### Инструмент 1: GET (Получение событий)
...
- **Настройки**:**:**
  ...
  - **Имя:** `get_events`
  - **Описание:** Опишите, когда использовать этот инструмент. Например: `Use this when you need to get events`
...
#### Инструмент 2: POST (Создание события)
...
#### Инструмент 3: PUT (Обновление события)
...
#### Инструмент 4: DELETE (Удаление события)
...
- Агент может динамически выбрать, какой инструмент использовать, исходя из запроса пользователя или логики сценария.
- Каждый инструмент сопоставлен с конкретным HTTP-методом и конечной точкой, с четко определенными схемами ввода.
...
Вот несколько **примеров взаимодействий** для вашего потока, включая примеры пользовательских запросов и ожидаемое поведение для каждого из них, сопоставленное с соответствующим инструментом (GET, POST, PUT, DELETE):
...
Инструменты Requests из 4-х работают отлично, если у вас всего несколько API, но представьте, что у вас десятки или сотни API — это может стать трудно поддерживать. Чтобы решить эту проблему, osmi предоставляет набор инструментов OpenAPI, который способен взять YAML-файл OpenAPI и преобразовать каждый API в отдельный инструмент. Спецификация OpenAPI (OAS) — это универсальный стандарт для описания деталей RESTful API в формате, который может читать и интерпретировать машина.
...
Далее добавьте узел Агент. В этой конфигурации агент настроен только с одним инструментом — OpenAPI Toolkit.

#### Инструмент: OpenAPI Toolkit

![](/assets/image%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\)%20\(1\).png){width="332"}

- **Цель:** Получить список API из YAML-файла и преобразовать каждый API в список инструментов.
...
Из приведённых выше примеров мы увидели, как агент может динамически вызывать инструменты и взаимодействовать с API. В некоторых случаях может потребоваться последовательный вызов API до или после определённых действий. Например, вы можете получить список клиентов из CRM и передать его агенту. В таких случаях можно использовать узел [HTTP](../using-flowise/agentflowv2#id-6.-http-node).
